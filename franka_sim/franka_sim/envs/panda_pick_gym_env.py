...
class PandaPickCubeGymEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        obs_image_size: Tuple[int, int] = (128, 128),
    ):
        ...
        if self.image_obs:
            obs_h, obs_w = self.obs_image_size
            self.observation_space = gym.spaces.Dict(
                {
                    "state": gym.spaces.Dict(
                        {
        ...
        # Separate renderer for observation images (smaller size for efficiency)
        obs_h, obs_w = self.obs_image_size
        self._obs_viewer = MujocoRenderer(
            self.model,
            self.data,
            width=obs_w,
            height=obs_h,
    ...
    def reset(
        ...
        steps: 
            1. 重置 mujoco model 和 data
            2. 重置 arm 到 home 位置(mj_forward 到初始joints)
            3. 重置 mocap body 到 home 位置
                背景: 采用的 mocap 控制方案(mocap 是一个虚拟 body, 用来给控制器目标位置),在底层 opspace 控制器中计算输出力矩
                背景的背景: 为什么目标位置不用实际 forward 的值? 在这个 reset 场景下他们两个都可。
                            但是在真正控制的时候, mocap 相当于目标值, forward相当于当前值(在真正控制的时候也不会使用forward函数了说实话)
            4. 采样一个新的 block 位置
            5. 缓存初始 block 高度(用于计算reward)
            6. 计算 observation
        """
    ...
    def step(
        ...
        obs = self._compute_observation()
        rew, success = self._compute_reward()
    ...
    def _compute_observation(self) -> dict:
        ...
        if self.image_obs:
            obs["images"] = {}
            # Use smaller obs_viewer for efficient observation images
            self._obs_viewer.camera_id = self.camera_id[0]
            obs["images"]["front"] = self._obs_viewer.render(render_mode="rgb_array")
            self._obs_viewer.camera_id = self.camera_id[1]
            obs["images"]["wrist"] = self._obs_viewer.render(render_mode="rgb_array")
        else:
            block_pos = self._data.sensor("block_pos").data.astype(np.float32)
            obs["state"]["block_pos"] = block_pos

        if self.render_mode == "human":
            # Use full-size viewer for human display
    ...
    def _compute_reward(self) -> float:
        """
        Compute the reward.
        连续奖励: pos空间的距离越小, block被抬起的"成功比例"越大, reward越大
        """
